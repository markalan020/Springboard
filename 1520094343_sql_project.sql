...
WHERE memid = (SELECT max(memid) FROM country_club.Members)
...
SELECT DISTINCT Facilities.name, concat(Members.firstname,' ', Members.surname) as mem_name
FROM country_club.Members Members, country_club.Facilities Facilities, country_club.Bookings Bookings
WHERE
...
SELECT * 
FROM (SELECT Facilities.name, concat(Members.firstname,' ', Members.surname) as mem_name, 
CASE WHEN Bookings.memid = 0 THEN Facilities.guestcost * Bookings.slots
ELSE Facilities.membercost * Bookings.slots END AS total_cost
FROM country_club.Members Members, country_club.Facilities Facilities, country_club.Bookings Bookings
WHERE
Bookings.facid = Facilities.facid
AND Members.memid = Bookings.memid 
AND Bookings.starttime LIKE '2012-09-14%') as costing
WHERE total_cost > 30
ORDER BY total_cost desc
...
AND Members.memid = Bookings.memid
GROUP BY Facilities.name) as revenues
WHERE revenue < 1000
